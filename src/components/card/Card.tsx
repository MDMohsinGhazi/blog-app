...
const Card: React.FC<CardProps> = ({ id, title, imageUrl, textContent }) => {
    return (
        <div className="w-80 self-stretch p-4 bg-white rounded-lg shadow-md transform hover:scale-105 transition-transform duration-300 ease-in-out">
            ...
            <div className="pt-4 pb-2">
                ...
                <div className="flex justify-between items-center mt-4">
                    <Link
                        href={`/blog/${id}`}
                        className="bg-primary hover:bg-primary/90 text-white px-4 py-1 rounded-full focus:outline-none"
                    >
                        Read more
                    </Link>
                </div>
